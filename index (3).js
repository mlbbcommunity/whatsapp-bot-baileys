    ...
    async handleConnectionUpdate(update) {
        ...
        if (qr && !this.isConnected && !this.pairingCodeGenerated) {
            try {
                ...
                console.log('=================================\n');
                console.log('📱 CRITICAL PAIRING STEPS:');
                console.log('1. First, LOG OUT of ALL WhatsApp Web sessions');
                ...
                console.log('5. Remove any existing "WhatsApp Web" devices');
                console.log('6. Tap "Link a Device"');
                console.log('7. Tap "Link with phone number instead"');
                console.log('8. Enter this exact code: ' + code);
                console.log('9. Complete pairing within 90 seconds');
                console.log('=================================\n');
                console.log('⚠️  MUST DO BEFORE PAIRING:');
                console.log('- Log out of ALL other WhatsApp Web sessions');
                console.log('- Switch to mobile data (not WiFi)');
                console.log('- Close all WhatsApp Web browser tabs');
        ...
        if (connection === 'close') {
            ...
            // Check for specific disconnection reasons
            const isConnectionFailure = lastDisconnect?.error?.message?.includes('Connection Failure');
            const isAuthFailure = errorCode === DisconnectReason.connectionClosed || 
                                 errorCode === DisconnectReason.connectionLost ||
                                 errorCode === DisconnectReason.connectionReplaced;
            
            if (isAuthFailure) {
                logger.warn('Authentication or session issue detected. Clearing session data...');
                await this.clearSessionData();
                this.reconnectAttempts = 0;
            ...
            if (shouldReconnect && this.reconnectAttempts < this.maxReconnectAttempts) {
                ...
                if (isConnectionFailure && this.reconnectAttempts >= 2) {
                ...
                logger.warn('Max reconnection attempts reached. Clearing session data and restarting...');
                await this.clearSessionData();
